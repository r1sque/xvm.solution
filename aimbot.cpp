...
void Aimbot::find(BoneArray* bones) {
	...
	if (found_target) {
		...
		// set member vars.
		if (best.player && best.record && best.damage > 0) {
...
bool AimPlayer::SetupHitboxPoints(LagRecord* record, BoneArray* bones, int index, std::vector<vec3_t>& points) {
	...
	// get hitbox scales.
	float scale = g_menu.main.aimbot.head_scale.get() / 100.f;
	float bscale = g_menu.main.aimbot.body_scale.get() / 100.f;

	// Zeus weapon-specific scaling.
	if (g_cl.m_weapon_id == ZEUS) {
		scale = bscale = 0.7f;
	}

	// Adjust scales if the player is airborne.
	...
	if (bbox->m_radius <= 0.f) {
		...
		if (index == HITBOX_R_FOOT || index == HITBOX_L_FOOT) {
			...
			if (g_menu.main.aimbot.multipoint.get(3)) {
				...
				// heel.
				points.push_back({ center.x + ((bbox->m_mins.x - center.x) * scale), center.y, center.z });
			}
		}
	...
	// these hitboxes are capsules.
		...
		if (index == HITBOX_HEAD) {
			...
			if (g_menu.main.aimbot.multipoint.get(0)) {
				...
				if (record->m_velocity.length() <= 0.1f && record->m_eye_angles.x > 80.f) {
...
void Aimbot::apply() {
	...
	if (attack || attack2) {
		...
		if (m_target) {
			...
			g_cl.m_cmd->m_view_angles = math::look(g_cl.m_shoot_pos, m_aim);

			if (!g_menu.main.aimbot.silent.get())
				g_csgo.m_engine->SetViewAngles(g_cl.m_cmd->m_view_angles);
			...
			g_shots.OnShotFire(m_target, m_damage, g_cl.m_weapon_info->m_bullets, m_record, m_aim, m_hitbox, m_hitgroup);
		}
