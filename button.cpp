...
void Button::draw( ) {
	...
	// inner blob
		...
		if (g_input.IsCursorInRect(btn)) {
			...
			render::RoundedBoxStaticOutline(p.x + BUTTON_X_OFFSET + 1, p.y + 1, m_w - BUTTON_X_OFFSET - 2, BUTTON_BOX_HEIGHT - 2, 3, { 53,53,53, m_parent->m_alpha });
			render::RoundedBoxStaticOutline(p.x + BUTTON_X_OFFSET, p.y, m_w - BUTTON_X_OFFSET, BUTTON_BOX_HEIGHT, 3, { 15,15,15, m_parent->m_alpha });

			// button name
			render::menu.string(p.x + ((BUTTON_X_OFFSET + m_w) / 2),
				p.y + 5, { 205, 205, 205, m_parent->m_alpha }, m_label, render::ALIGN_CENTER);
}
