...
ulong_t __stdcall Client::init( void* arg ) {
	...
	g_cl.m_user = XOR("xvm.solution");
	g_cl.m_version = XOR("Ver 1.16");

	// welcome the user.
	g_notify.add(XOR("welcome to xvm.solution\n"));
...
void Client::DrawHUD( ) {
	...
	std::string current_time = time.str();
	...
	// get framerate  - Fix the watermark moving each time it updates the framrate
	float frametime = g_csgo.m_globals->m_frametime; // Time per frame in seconds.
	float fps = 1.f / frametime; 

	
	std::string ver = g_cl.m_version;

	std::string text = tfm::format(XOR("xvm.solution [zone 09] | Latency: %i ms | Time: %s | %s"), ms1, current_time.c_str(), ver);
	...
	// background.
	//render::rect_filled(m_width - size.m_width - 50, 10, size.m_width + 30, size.m_height + 15, { 27, 27, 27, 230 });
	//render::rect(m_width - size.m_width - 50, 10, size.m_width + 30, size.m_height + 15, color);

	render::RoundedBoxStatic(m_width - size.m_width - 50, 10, size.m_width + 30, size.m_height + 15, 4, { 27, 27, 27, 230 });
	render::RoundedBoxStaticOutline(m_width - size.m_width - 50, 10, size.m_width + 30, size.m_height + 15, 4, color);

	// text.
	render::hud.string(m_width - 35, 19, color, text, render::ALIGN_RIGHT);
...
void Client::print( const std::string text, ... ) {
	...
	// print to console.
	g_csgo.m_cvar->ConsoleColorPrintf(color, XOR("[xvm.sln] ") );
