...
void __fastcall eax::modify_eye_pos( CCSGOPlayerAnimState* ecx, std::uintptr_t edx, vec3_t& pos) {
	// remove call from client
	return;
}

void __fastcall eax::calc_view(void* ecx, const std::uintptr_t edx, vec3_t& eye_origin, const ang_t& eye_ang, float& z_near, float& z_far, float& fov) {
...
void __fastcall eax::update_client_side_anim( Player* const player, const std::uintptr_t edx) {
	if (!player
		|| !player->alive()
		|| !player->networkable()
		|| player->index() < 1
	...
	if (!g_cl.m_update) {
		...
		if (player->m_bIsLocalPlayer()) {
			g_ServerAnimations.HandleServerAnimation(g_cl.m_cmd);
		}
		return;
	}
...
void __fastcall eax::interpolate_server_entities() {

	if (!g_cl.m_local || !g_csgo.m_engine->IsInGame())
		return g_hooks.m_InterpolateServerEntities();

	g_hooks.m_InterpolateServerEntities();

	auto pStudioHdr = g_cl.m_local->m_studioHdr();

	// fix server model origin
	{
		g_cl.m_local->SetAbsAngles(ang_t(0.0f, g_cl.m_abs_yaw, 0.0f));

		g_cl.m_local->InvalidateBoneCache();

		auto backuped_bones = g_cl.m_local->m_BoneAccessor().m_pBones;
		...
		if (pStudioHdr) {
			...
			g_csgo.m_AttachmentHelper.as< AttachmentHelperFn  >()(g_cl.m_local, pStudioHdr);
		}

		g_cl.m_local->m_BoneAccessor().m_pBones = backuped_bones;
	}

	for (int i = 1; i <= g_csgo.m_globals->m_max_clients; ++i) {
		const auto entity = (Player*)g_csgo.m_entlist->GetClientEntity(i);
		if (!entity)
			continue;
		...
		entity->SetupBones(nullptr, 128, 0x0007FF00, entity->m_flSimulationTime());
	}
}

void __fastcall eax::do_extra_bones_processing( Player* const ecx, const std::uintptr_t edx, int a0, int a1, int a2, int a3, int a4, int a5) {
	return;
}

void __fastcall eax::build_transformations( Player* ecx, void* edx, CStudioHdr* hdr, vec3_t* pos, quaternion_t* q, matrix3x4_t* cam_transform, int bone_mask, byte* computed )
{
...
bool __fastcall eax::should_skip_animation_frame(void* this_pointer, void* edx) {
...
void __fastcall eax::check_for_sequence_change(void* this_pointer, void* edx, void* hdr, int cur_sequence, bool force_new_sequence, bool interpolate) {
...
void __fastcall eax::standard_blending_rules( Player* const ecx, const std::uintptr_t edx, CStudioHdr* const mdl_data, int a1, int a2, float a3, int mask) {
	if ( !ecx
		|| ecx->index() < 1
		|| ecx->index() > 64)
...
void Hooks::init( ) {
	...
	static auto interpolate_server_entities = (DWORD)pattern::find(g_csgo.m_client_dll, XOR("55 8B EC 83 EC 1C 8B 0D ? ? ? ? 53 56"));
	...
	hook( modif_eye_pos_addr, eax::modify_eye_pos, eax::orig_modify_eye_pos);
	hook( calc_view_addr, eax::calc_view, eax::orig_calc_view);
	hook( update_csa_addr, eax::update_client_side_anim, eax::orig_update_client_side_anim);
	hook( do_extra_bone_proc_addr, eax::do_extra_bones_processing, eax::orig_do_extra_bones_processing);
	hook( build_trans_addr, eax::build_transformations, eax::orig_build_transformations);
	hook( check_seq_change_addr, eax::check_for_sequence_change, eax::orig_check_for_seq_change);
	hook( skip_frame_addr, eax::should_skip_animation_frame, eax::orig_should_skip_animation_frame);
	hook( standard_blending_rules_addr, eax::standard_blending_rules, eax::orig_standard_blending_rules);
	MH_CreateHook( to_byte interpolate_server_entities, eax::interpolate_server_entities, reinterpret_cast<void**>(&m_InterpolateServerEntities));
