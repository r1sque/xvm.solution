...
void Keybind::draw( ) {
	...
	//render::RoundedBoxStaticOutline(p.x + KEYBIND_X_OFFSET + 1, p.y + 15, m_w - KEYBIND_X_OFFSET - 2, KEYBIND_BOX_HEIGHT - 2, 3, { 53,53,53, m_parent->m_alpha });
	//render::RoundedBoxStaticOutline(p.x + KEYBIND_X_OFFSET, p.y + 15, m_w - KEYBIND_X_OFFSET, KEYBIND_BOX_HEIGHT, 3, { 15,15,15, m_parent->m_alpha });

	// inner blob
	// todo; gradient?
	render::rect_filled( p.x + KEYBIND_X_OFFSET + 1, p.y + 15 + 1, m_w - KEYBIND_X_OFFSET - 2, KEYBIND_BOX_HEIGHT - 2, { 41, 41, 41, m_parent->m_alpha } );
	//render::gradient2(p.x + KEYBIND_X_OFFSET + 1, p.y + 15 + 1, m_w - KEYBIND_X_OFFSET, KEYBIND_BOX_HEIGHT, { 40, 40, 40, m_parent->m_alpha }, { 33, 33, 33, m_parent->m_alpha });
	//render::RoundedBoxStatic(p.x + KEYBIND_X_OFFSET + 1, p.y + 15 + 1, m_w - KEYBIND_X_OFFSET - 2, KEYBIND_BOX_HEIGHT - 2, 3, { 41, 41, 41, m_parent->m_alpha });
	
	// todo; animate with '...'
	if( m_set )
		render::menu.string( p.x + KEYBIND_X_OFFSET + KEYBIND_ITEM_X_OFFSET, p.y + 15 + 1, color, XOR( "press key" ) );

	// we have a key assigned.
	else if( m_key >= 0 && m_key <= 0xFE )
		render::menu.string( p.x + KEYBIND_X_OFFSET + KEYBIND_ITEM_X_OFFSET, p.y + 15 + 1, { 152, 152, 152, m_parent->m_alpha }, keynames[ m_key ] );
}
