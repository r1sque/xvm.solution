...
public:
	void init( ) {
		...
		manual_anti_aim_col.setup(XOR("manual anti-aim color"), XOR("manual_anti_aim_col"), { 255, 255, 255 });
		RegisterElement(&manual_anti_aim_col, 1);

		fake_yaw_indicator.setup(XOR("anti-aim lines"), XOR("anti_aim_line"));
		RegisterElement(&fake_yaw_indicator, 1);
...
public:
	void init( ) {
		...
		esp_style.setup(XOR("indicator style"), XOR("esp_style"), { XOR("default"), XOR("unique"), XOR("wilaya"), XOR("ot style") });
		...
		menu_color.setup(XOR("menu color"), XOR("menu_color"), { 245, 239, 186, 255 }, &g_gui.m_color);
...
public:
	void init() {
		...
		ver.setup(XOR("Cheat Version 1.16"));
		ver.SetCallback(callbacks::emporium);
		RegisterElement(&ver);


		stuff.setup(XOR("What is new"), XOR("stuff"), 0.0f, 2, true, 0.0, 2.0f, 1.0f);
		RegisterElement(&stuff);

		text.setup(XOR("[+] improved Aimbot logic"), XOR("fast_stop"));
		RegisterElement(&text);

		text_1.setup(XOR("[+] improved Resolver"), XOR("fast_stop"));
		RegisterElement(&text_1);

		text_2.setup(XOR("[+] improved Hitbox selection"), XOR("fas_stop"));
		RegisterElement(&text_2);

		text_3.setup(XOR("[+] updated Spectator List style"), XOR("fas_stop"));
		RegisterElement(&text_3);

		text_4.setup(XOR("[+] added new Indicator style"), XOR("fas_stop"));
		RegisterElement(&text_4);

		text_5.setup(XOR("[-] removed DT from Ver 1.14"), XOR("fas_stop"));
		RegisterElement(&text_5);

		text_6.setup(XOR("[+] added a lot and improved Multipoints"), XOR("fas_stop"));
		RegisterElement(&text_6);


		ver1.setup(XOR("Stuff To Do"));
		ver1.SetCallback(callbacks::die);
		RegisterElement(&ver1, 1);

		text_1_0.setup(XOR("improve the resolver more"), XOR("fast_stop"));
		RegisterElement(&text_1_0, 1);

		text_1_1.setup(XOR("add dynamic weapons"), XOR("fast_stop"));
		RegisterElement(&text_1_1, 1);

		text_1_2.setup(XOR("finish refining indicator"), XOR("fas_stop"));
		RegisterElement(&text_1_2, 1);

		text_1_3.setup(XOR("indicator styles logic"), XOR("fas_stop"));
		RegisterElement(&text_1_3, 1);

		text_1_4.setup(XOR("Make keybind toggle, hold, etc..."), XOR("fas_stop"));
		RegisterElement(&text_1_4, 1);

		text_1_6.setup(XOR("Fix auto peek"), XOR("fas_stop"));
		RegisterElement(&text_1_6, 1);

		text_1_7.setup(XOR("try menu styles"), XOR("fas_stop"));
		RegisterElement(&text_1_7, 1);
