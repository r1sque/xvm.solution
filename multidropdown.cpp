	...
	if( !m_items.empty( ) ) {
		...
		if( !m_active_items.size( ) )
			render::menu.string( p.x + DROPDOWN_X_OFFSET + DROPDOWN_ITEM_X_OFFSET, 
								   p.y + m_offset + 1, { 152, 152, 152, m_parent->m_alpha }, XOR( "-" ) );

		// one item.
		else if( m_active_items.size( ) == 1 )
			render::menu.string( p.x + DROPDOWN_X_OFFSET + DROPDOWN_ITEM_X_OFFSET, 
								 p.y + m_offset + 1, { 152, 152, 152, m_parent->m_alpha }, m_items[ m_active_items.front( ) ] );
		...
		// more then one item.
			...
			render::menu.string( p.x + DROPDOWN_X_OFFSET + DROPDOWN_ITEM_X_OFFSET, p.y + m_offset + 1, { 152, 152, 152, m_parent->m_alpha }, ss.str( ) );
		}
	}
}
