...
void Visuals::Spectators() {
	...
	// Spectator list initialization
	std::vector<std::string> spectators{ XOR("spectators") };
	int text_height = render::menu_shade.m_size.m_height;

	// Iterate over all players to find spectators
	for (int i{ 1 }; i <= g_csgo.m_globals->m_max_clients; ++i) {
		Player* player = g_csgo.m_entlist->GetClientEntity<Player*>(i);
		if (!player || player->m_bIsLocalPlayer() || player->dormant() || player->alive())
			continue;
	...
	int box_width = 200;													// Box width
	int max_box_height = 300;												// Maximum height of the box
	int dynamic_box_height = (spectators.size() * (text_height + 4)) + 25;	// Dynamic height
	int box_height = std::min(dynamic_box_height, max_box_height);			// Clamp to max height
	static int x_pos = g_cl.m_width - box_width - 20;						// Right-aligned with padding
	static int y_pos = (g_cl.m_height / 2) - (box_height / 2);				// Centered vertically
	...
	if (m_drag) {
		if (g_input.GetKeyState(VK_LBUTTON)) {
			...
			// Update the position with respect to the drag offset
			x_pos = g_input.m_mouse.x - drag_offset_x;
			y_pos = g_input.m_mouse.y - drag_offset_y;

			// Clamp the position to screen bounds
			math::clamp(x_pos, 0, g_cl.m_width - box_width);
			math::clamp(y_pos, 0, g_cl.m_height - box_height);
		}
		else {
		...
		// Check for initial drag condition (mouse click on the top border)
		if (g_input.GetKeyPress(VK_LBUTTON)) { // Check for mouse button press
			g_csgo.m_input_system->GetCursorPosition(&g_input.m_mouse.x, &g_input.m_mouse.y);

			// If the click is within the draggable area of the box, start dragging
			if ((g_input.m_mouse.x >= x_pos && g_input.m_mouse.x <= x_pos + box_width &&
				 g_input.m_mouse.y >= y_pos && g_input.m_mouse.y <= y_pos + 10) ||							 // Top border

				(g_input.m_mouse.x >= x_pos && g_input.m_mouse.x <= x_pos + 10 &&
				 g_input.m_mouse.y >= y_pos && g_input.m_mouse.y <= y_pos + box_height) ||					 // Left border

				(g_input.m_mouse.x >= x_pos + box_width - 10 && g_input.m_mouse.x <= x_pos + box_width &&
				 g_input.m_mouse.y >= y_pos && g_input.m_mouse.y <= y_pos + box_height) ||					 // Right border

				(g_input.m_mouse.x >= x_pos && g_input.m_mouse.x <= x_pos + box_width &&
				 g_input.m_mouse.y >= y_pos + box_height - 10 && g_input.m_mouse.y <= y_pos + box_height)) { // Bottom border

				m_drag = true;
				drag_offset_x = g_input.m_mouse.x - x_pos;
				drag_offset_y = g_input.m_mouse.y - y_pos;
			}
		}
	}

	// Render the box background
	render::rect_filled(x_pos, y_pos, box_width, box_height, { 0, 0, 0, 170 }); // Transparent background

	// Render the title "spectators" at the top of the box
	render::menu_shade.string(
		x_pos + (box_width / 2),							// Centered horizontally
		y_pos + 10,											// Fixed vertical position
		{ 255, 255, 255, 255 },								// White text color
		XOR("spectators"),									// Title text
		render::ALIGN_CENTER								// Center-aligned text
	);

	// Centered line under "spectators"
	int line_width = box_width - 40;						// Original line width
	int line_y_pos = y_pos + 30;							// Line directly below "spectators" title
	int line_start = x_pos + (box_width - line_width) / 2;	// Calculate centered start position

	render::line(
		line_start,											// Start position (centered)
		line_y_pos,											// Fixed vertical position
		line_start + line_width,							// End position (adjusted for width)
		line_y_pos,											// Fixed vertical position
		color												// Line color
	);

	// Render box border (multi-layered for depth)
	render::rect(x_pos, y_pos, box_width, box_height, { 5, 5, 5, 255 }); // Outer border
	render::rect(x_pos + 1, y_pos + 1, box_width - 2, box_height - 2, { 60, 60, 60, 100 });
	render::rect(x_pos + 2, y_pos + 2, box_width - 4, box_height - 4, { 40, 40, 40, 100 });
	render::rect(x_pos + 3, y_pos + 3, box_width - 6, box_height - 6, { 40, 40, 40, 100 });
	render::rect(x_pos + 4, y_pos + 4, box_width - 8, box_height - 8, { 40, 40, 40, 100 });
	render::rect(x_pos + 5, y_pos + 5, box_width - 10, box_height - 10, { 60, 60, 60, 100 });
	render::rect(x_pos + 6, y_pos + 6, box_width - 12, box_height - 12, { 5, 5, 5, 255 }); // Inner border

	// Render spectator names inside the box
	for (size_t i{ 1 }; i < spectators.size(); ++i) {
		const std::string& name = spectators[i];

		render::menu_shade.string(
			x_pos + (box_width / 2),                     // Centered horizontally
			y_pos + 40 + ((i - 1) * (text_height + 4)),  // Start 40px from top, dynamic spacing
			{ 255, 255, 255, 255 },                      // White text color
			name,
			render::ALIGN_CENTER                         // Center-aligned text
		);
...
void Visuals::StatusIndicators() {
	...
	if (g_menu.main.misc.esp_style.get() == 1) {
		...
		for (size_t i{ }; i < indicators.size(); ++i) {
			...
			float centered = render::hud.size(indicator.text).m_width / 2.f;
			render::hud.string(x - std::round(centered * (1.f - (add_ / 20.f))), g_cl.m_height / 2 + 75 + (15 * i), indicator.color, indicator.text);
		}
	}

	if (g_menu.main.misc.esp_style.get() == 2) {
		// DMG
		if (g_aimbot.m_damage_toggle) {
			Indicator_t ind{ };
			ind.color = Color(250, 90, 111);
			ind.text = tfm::format(XOR("MINIMO DEL MINIMO DMG %i"), g_menu.main.aimbot.override_dmg_value.get());
			indicators.push_back(ind);
		}
		...
		if (g_aimbot.m_force_body) {
			Indicator_t ind{ };
			ind.color = Color(168, 60, 250);
			ind.text = XOR("PELVIS PRIORITY");
			indicators.push_back(ind);
		}
		...
		if (g_aimbot.m_fake_latency || g_aimbot.m_fake_latency2) {
			Indicator_t ind{ };
			float ping = g_aimbot.m_fake_latency2 ? g_menu.main.misc.secondary_fake_latency_amt.get() : g_menu.main.misc.fake_latency_amt.get();
			Color r = g_aimbot.m_fake_latency2 ? 77 : 77;
			Color g = g_aimbot.m_fake_latency2 ? 128 : 227;
			Color b = g_aimbot.m_fake_latency2 ? 238 : 238;
			ind.color = LerpRGB(Color(255, 0, 0), Color(r, g, b), std::clamp((g_csgo.m_cl->m_net_channel->GetLatency(INetChannel::FLOW_INCOMING) * 2000.f) / ping, 0.f, 1.f));
			ind.text = XOR("DDOS ON PEAK");
			indicators.push_back(ind);
		}
		...
		if (g_cl.m_local->m_vecVelocity().length_2d() > 270.f || g_cl.m_lagcomp) {
			Indicator_t ind{ };
			ind.color = g_cl.m_lagcomp ? Color(250, 143, 60) : Color(255, 0, 0);
			ind.text = XOR("RESOLVER CONFUSER");
		...
		Indicator_t ind{ };
		ind.color = change > 35.f ? Color(230, 229, 16) : Color(255, 0, 0);
		ind.text = XOR("VIETNAMESE DESYNC ANGLE");
		indicators.push_back(ind);
	...
	if (g_menu.main.misc.esp_style.get() == 3) {
		// DMG
		if (g_aimbot.m_damage_toggle) {
			Indicator_t ind{};
			ind.color = Color(250, 90, 111);
			ind.text = tfm::format(XOR("Minimum Damage: %i"), g_menu.main.aimbot.override_dmg_value.get());
			indicators.push_back(ind);
		}
		...
		int max_box_height = 300;												// Maximum height of the box
		int text_height = render::menu_shade.m_size.m_height;
		int dynamic_box_height = (indicators.size() * (text_height + 4)) + 45;	// Dynamic height
		int box_height = std::min(dynamic_box_height, max_box_height);			// Clamp to max height
		static int x_pos = 150;													// Left-aligned with padding
		static int y_pos = (g_cl.m_height / 2) - (box_height / 2);				// Centered vertically
		...
		if (m_drag) {
			...
			if (g_input.GetKeyPress(VK_LBUTTON)) {
				g_csgo.m_input_system->GetCursorPosition(&g_input.m_mouse.x, &g_input.m_mouse.y);
				if ((g_input.m_mouse.x >= x_pos && g_input.m_mouse.x <= x_pos + box_width &&						// Top border
					 g_input.m_mouse.y >= y_pos && g_input.m_mouse.y <= y_pos + 10) ||

					(g_input.m_mouse.x >= x_pos && g_input.m_mouse.x <= x_pos + 10 &&
					 g_input.m_mouse.y >= y_pos && g_input.m_mouse.y <= y_pos + box_height) ||						// Left border

					(g_input.m_mouse.x >= x_pos + box_width - 10 && g_input.m_mouse.x <= x_pos + box_width &&		// Right border
					 g_input.m_mouse.y >= y_pos && g_input.m_mouse.y <= y_pos + box_height) ||
		...
		// Render the title "indicators" at the top of the box
		render::menu_shade.string(
			x_pos + (box_width / 2),			// Centered horizontally
			y_pos + 10,							// Fixed vertical position
			{ 255, 255, 255, 255 },				// White text color
			XOR("indicators"),					// Title text
			render::ALIGN_CENTER				// Center-aligned text
		);

		// Centered line under "indicators"
		int line_width = box_width - 40;						// Original line width
		int line_y_pos = y_pos + 30;							// Line directly below "indicators" title
		int line_start = x_pos + (box_width - line_width) / 2;	// Calculate centered start position
		...
		// Render indicators inside the box
		for (size_t i = 0; i < indicators.size(); ++i) {
			const auto& indicator = indicators[i];

			render::menu_shade.string(
				x_pos + (box_width / 2),						// Centered horizontally
				y_pos + 40 + (i * (text_height + 4)),			// Start 40px from top, dynamic spacing
	...
	if (g_menu.main.visuals.fake_yaw_indicator.get()) {
		...
		if (render::WorldToScreen(pos, tmp))
		{
			...
			if (g_menu.main.antiaim.fake_yaw.get() == 1 || g_menu.main.antiaim.fake_yaw.get() == 2 || g_menu.main.antiaim.fake_yaw.get() == 3 || g_menu.main.antiaim.fake_yaw.get() == 4 || g_menu.main.antiaim.fake_yaw.get() == 5 || g_menu.main.antiaim.fake_yaw.get() == 6 || g_menu.main.antiaim.fake_yaw.get() == 7);
			{
...
void Visuals::ManualAntiAim() {
	...
	if (g_menu.main.misc.esp_style.get() == 1) {
		...
		for (size_t i{ }; i < indicators.size(); ++i) {
			...
			float centered = render::manualaa2.size(indicator.text).m_width / 2.f;
			render::manualaa2.string(x - std::round(centered * (1.f - (add_ / 20.f))), g_cl.m_height / 2 + 60 + (15 * i), indicator.color, indicator.text);
		}
	...
	if (g_menu.main.misc.esp_style.get() == 2) {
		...
		if (g_hvh.m_left)
			render::manualaa.string(x - 61, y - 8, color, "o");

		if (g_hvh.m_right)
			render::manualaa.string(x + 37, y - 8, color, "p");

		if (g_hvh.m_back)
			render::manualaa.string(x - 9, y + 35, color, "n");

		if (g_hvh.m_forward)
			render::manualaa.string(x - 9, y - 50, color, "m");
...
void Visuals::PenetrationCrosshair() {
	...
	if (g_menu.main.visuals.pen_crosshair.get() && (g_cl.m_pen_data.m_pen || valid_player_hit))
		render::esp_small2.string(x + 3, y + 2, { final_color }, std::to_string(damage1337).c_str(), render::ALIGN_LEFT);
	if (g_cl.m_pen_data.m_damage > 1) {

		// maybe make it a cicle lowkey
...
//void Visuals::RenderHitboxPoints(LagRecord* record, Color col, float time) {
//    if (!g_menu.main.visuals.render_hitbox_points.get() || !record)
//        return;
//
//	const model_t*		model;
//	studiohdr_t*		hdr;
...
//            continue;
//
//        // Create a rotation matrix for the hitbox
//        matrix3x4_t matrix;
...
//
//        // Get the world positions of the hitbox corners
//        vec3_t mins, maxs;
//        math::VectorTransform(bbox->m_mins, matrix, mins);
//        math::VectorTransform(bbox->m_maxs, matrix, maxs);
//
//        // Render capsule overlay for better visualization
//        g_csgo.m_debug_overlay->AddCapsuleOverlay(mins, maxs, bbox->m_radius, col.r(), col.g(), col.b(), col.a(), time, 0, 1);
//
//        // Render points on the hitbox
//        std::vector<vec3_t> points = {
//            mins,
//            maxs,
//            (mins + maxs) * 0.5f  // center point
//        };
//
//        for (const auto& point : points) {
//			vec2_t screen_point;
//            if (render::WorldToScreen(point, screen_point)) {
//                render::rect_filled(screen_point.x - 2, screen_point.y - 2, 4, 4, col);
//            }
//        }
//    }
//}
